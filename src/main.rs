...
pub fn think(info: &AgentInfo, heatmap: &Array2D<f32>, gamestate: &mut GameState) -> Command {
	...
	if bee.target.as_ref().is_none() {
		...
		if bee.has_role(Role::Build) {
			let build_position = find_build_position(info.bee, info.player);
			bee.set_target(build_position);
		} else {
	...
	if bee.role.as_ref().unwrap().eq(&Role::Build) {
		...
		if !bee.position.eq(&bee.target.unwrap()) {
			let command: Option<Command> = pathfind(info, &gamestate.map, bee.target.as_ref().unwrap());
			match command {
				Some(v) => {
					return v;
				},
		...
		let empty_cell = find_available_adjacent(bee.position, &gamestate.map.cells);
		match empty_cell {
			Some(v) => {
				return Command {
					action: Action::BUILD,
					direction: get_direction(&v, &bee.position).unwrap(),
				}
			},
			None => {
				return Command {
					action: Action::MOVE,
					direction: get_direction(&opponent_hive_coords(info.player), &bee.position).unwrap(),
				}
			},
		}
	}
