...
pub fn pathfind(info: &AgentInfo, map: &Map, destination: &Coords) -> Option<Direction> {
	...
	while !open_set.is_empty() {
		...
		if current == *destination {
			...
			while temp != coords {	
				// iterate through the came_from map and find the path
				path.push(came_from.get(&temp).unwrap().clone());
				came_from.remove(&came_from.keys().next().unwrap().clone());
			}
			debug_coords = temp;*/

			// Currently this has no connection to the pathfinding algorithm.
			// It needs to be something like (current, node_from_path)
			return Some(coords_to_dir(coords, *destination));
		}
		for direction in Direction::into_enum_iter() {
